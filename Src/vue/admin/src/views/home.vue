...
<div class="content" :style='{"padding":"20px 30px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20231003/cc88bae96def417689c07dc1358d4a32.png) no-repeat center top / cover","display":"flex","fontSize":"14px","height":"100%"}'>
	...
	<div :style='{"border":"0px solid rgba(126, 96, 16, .1)","padding":"0px 0","margin":"0 0px 20px 0px","alignItems":"center","color":"#62779c","flexWrap":"wrap","background":"none","display":"flex","width":"100%","fontSize":"14px","order":"0"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"12px 0","margin":"0 2% 20px 0","borderColor":"rgba(126, 96, 16, .1)","alignItems":"center","textAlign":"left","flexDirection":"row-reverse","display":"flex","borderRadius":"8px","background":"url(http://codegen.caihongy.cn/20231003/b8ae010ba2ff44268dee3545917b55dd.png) no-repeat right bottom / 80% 80%,#fff","borderWidth":"0 0 0px","width":"18%","borderStyle":"solid"}' v-if="isAuth('xuexijindu','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"34px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{xuexijinduCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>学习进度总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 2 -->
	<div class="type2" :style='{"padding":"0","margin":"0","display":"flex","justifyContent":"space-between","alignContent":"flex-start","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","width":"100%","position":"relative","height":"420px","order":"10"}'>
		<div id="xuexijinduChart1" class="echarts1" v-if="isAuth('xuexijindu','首页统计')"></div>
		<div id="xuexijinduChart2" class="echarts2" v-if="isAuth('xuexijindu','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getxuexijinduCount();
		this.xuexijinduChat1();
		this.xuexijinduChat2();
	},
	methods:{
		// 词云
		...
		getxuexijinduCount() {
			this.$http({
				url: `xuexijindu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.xuexijinduCount = data.data
				}
			})
		},
		xuexijinduChat1() {
        ...
        this.$http({
            url: `xuexijindu/value/kechengmingcheng/xuexishizhang`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
    ...
    xuexijinduChat2() {
      this.$nextTick(()=>{
        ...
        var xuexijinduChart2 = echarts.init(document.getElementById("xuexijinduChart2"),'macarons');
        this.$http({
            url: "xuexijindu/group/xuexijindu",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                xuexijinduChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    xuexijinduChart2.resize();
                };
            }
        });
      })
