...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="课程类型" prop="courseType">
            <el-input v-model="ruleForm.courseType" 
                placeholder="课程类型" clearable :disabled=" false  ||ro.courseType"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`courseType/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('courseType/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`courseType/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
