<template>
	<div class="page-container">
		...
		<div class="back-container" v-if="centerType">
			<el-button size="mini" @click="backClick" type="warning" icon="el-icon-back">返回</el-button>
		</div>
		...
		<div class="list-main-container">
			<el-form :inline="true" :model="formSearch" class="search-form">
				...
				<el-button v-if="btnAuth('kechengxinxi', '新增')" class="add-btn" type="primary"
					@click="add('/index/kechengxinxiAdd')">
					...
					<div class="course-item" v-for="(item, index) in dataList" :key="index"
						...
						<div class="course-image">
							<img @click.stop="imgPreView(item.kechengtupian)"
								:src="item.kechengtupian && item.kechengtupian.substr(0, 4) == 'http' ? item.kechengtupian.split(',')[0] : baseUrl + (item.kechengtupian ? item.kechengtupian.split(',')[0] : '')"
								class="image" />
						</div>
						<div class="course-info">
							<div class="info-main">
								...
								<div class="course-price" v-if="item.price">
									<span class="price-symbol">￥</span>{{ item.price }}
								</div>
								<div class="course-meta">
									<div class="meta-item">
										<span class="icon iconfont icon-zan10"></span>
										<span class="meta-text">{{ item.thumbsupnum }}</span>
									</div>
									<div class="meta-item">
										<span class="icon iconfont icon-shoucang10"></span>
										<span class="meta-text">{{ item.storeupnum }}</span>
									</div>
									<div class="meta-item">
										<span class="icon iconfont icon-chakan9"></span>
										<span class="meta-text">{{ item.clicknum }}</span>
									</div>
								</div>
							</div>
							<div class="course-desc" v-html="item.kechengneirong"></div>
						</div>
					</div>
				</div>
			</div>
	...
	created() {
		...
		this.$http.get('option/kechengleixing/kechengleixing').then(res => {
			if (res.data.code == 0) {
				...
				this.selectOptionsList.push({ name: '课程类型', list: this.kechengleixingOptions, tableName: 'kechengleixing', check: -1 })
			}
		});
		this.getFenlei();
		this.getList(1, '全部');
