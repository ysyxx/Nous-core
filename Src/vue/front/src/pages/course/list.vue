<template>
	<div>
		...
		<div class="list-preview"
			:style='{ "padding": "0", "margin": "0px auto", "flexWrap": "wrap", "background": "#fff", "display": "flex", "width": "100%", "position": "relative" }'>



			<el-form :inline="true" :model="formSearch" class="list-form-pv"
				:style='{ "padding": "0px 7% 20px", "margin": "20px 0 0", "borderColor": "#ddd", "alignItems": "center", "flexWrap": "wrap", "background": "none", "borderWidth": "0 0 1px", "display": "flex", "width": "100%", "borderStyle": "solid", "height": "auto", "order": "1" }'>
				<el-form-item :style='{ "margin": "0 10px" }'>
					<div class="lable" v-if="true"
						:style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "display": "inline-block" }'>课程名称：
					</div>
				...
				<el-form-item :style='{ "margin": "0 10px" }'>
					<div class="lable" v-if="true"
						:style='{ "width": "auto", "padding": "0 10px", "lineHeight": "42px", "display": "inline-block" }'>课程课时：
					</div>
					<el-input v-model="formSearch.kechengkeshi" placeholder="课程课时"
						@keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button v-if="true"
					:style='{ "cursor": "pointer", "border": "0", "padding": "0px 15px", "margin": "0 10px 0 0", "outline": "none", "color": "#fff", "borderRadius": "0px", "background": "#F5BB00", "width": "auto", "fontSize": "14px", "lineHeight": "36px", "height": "36px" }'
				...
				<el-button v-if="btnAuth('course', '新增')"
					:style='{ "cursor": "pointer", "border": "0", "padding": "0px 15px", "margin": "0 10px 0 0", "outline": "none", "color": "#fff", "borderRadius": "0px", "background": "#333", "width": "auto", "fontSize": "14px", "lineHeight": "36px", "height": "36px" }'
					type="primary" @click="add('/index/courseAdd')"><i v-if="true"
						:style='{ "color": "#fff", "margin": "0 10px 0 0", "fontSize": "14px" }'
			...
			<div class="select2"
				...
				<div :style='{ "padding": "0px", "margin": "4px 0", "borderColor": "#eee", "background": "none", "borderWidth": "0 0 1px", "width": "100%", "position": "relative", "borderStyle": "dashed", "height": "auto" }'
					class="list" v-for="(item, index) in selectOptionsList" :key="item">
					...
					<div :style='{ "width": "auto", "display": "inline-block", "height": "auto" }' class="item-body">
						<div class="item" @click="selectClick2(item, -1)" :class="item.check == -1 ? 'active' : ''">全部
						</div>
						<div class="item" @click="selectClick2(item, index1)"
							:class="item.check == index1 ? 'active' : ''" v-for="item1, index1 in item.list"
							:key="item1">{{ item1 }}</div>
					</div>
				</div>
			</div>
			<div class="sort_view"
				:style='{ "width": "100%", "padding": "0 7%", "margin": "20px auto 10px", "textAlign": "right", "background": "#fff", "order": "3" }'>
				<el-button :style='{ "border": "0", "padding": "0 15px", "margin": "0 5px", "borderRadius": "0px" }'
					@click="sortClick('clicknum')">
					<span :style='{ "margin": "0 2px 0 0", "lineHeight": "40px", "fontSize": "14px", "color": "#333" }'
						class="icon iconfont icon-xiaoliang13" v-if="sortType != 'clicknum'"></span>
					<span :style='{ "margin": "0 2px 0 0", "lineHeight": "40px", "fontSize": "14px", "color": "#333" }'
			...
			<div class="list"
				:style='{ "width": "100%", "padding": "0 7%", "margin": "10px auto", "background": "#fff", "order": "4" }'>

				<!-- 样式三 -->
				<div class="list3 index-pv1"
					:style='{ "padding": "0 0px", "overflow": "hidden", "flexWrap": "wrap", "background": "#fff", "display": "flex", "width": "100%", "clear": "both", "justifyContent": "space-between", "height": "auto" }'>
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)"
						class="list-item animation-box">
						<div :style='{ "border": "1px solid #f6f6f6", "width": "200px", "padding": "0px", "margin": "0 10px 0 0", "overflow": "hidden", "height": "200px" }'
							class="img-box">
							<img @click.stop="imgPreView(item.coverImageUrl)"
								:style='{ "width": "100%", "objectFit": "cover", "display": "block", "height": "100%" }'
								v-if="item.coverImageUrl && item.coverImageUrl.substr(0, 4) == 'http'"
								:src="item.coverImageUrl.split(',')[0]" class="image" />
							<img @click.stop="imgPreView(baseUrl + (item.coverImageUrl ? item.coverImageUrl.split(',')[0] : ''))"
								:style='{ "width": "100%", "objectFit": "cover", "display": "block", "height": "100%" }' v-else
								:src="baseUrl + (item.coverImageUrl ? item.coverImageUrl.split(',')[0] : '')"
								class="image" />
						</div>
						<div :style='{ "padding": "0 10px", "overflow": "hidden", "flex": "1", "display": "flex", "height": "auto" }'
							class="item-info">
							<div
								:style='{ "width": "50%", "padding": "0", "alignItems": "center", "flexWrap": "wrap", "flex": "1", "display": "flex" }'>
								<div class="title">{{ item.title }}</div>
								<div class="title">{{ item.kechengleixing }}</div>
								<div :style='{ "width": "100%", "padding": "2px 10px", "borderColor": "#f0f0f0", "borderStyle": "dashed", "borderWidth": "0 0 1px 0", "display": "none" }'
									class="time">
									<span class="icon iconfont icon-shijian21"></span>
									<span class="text">{{ item.addtime }}</span>
								</div>
								<div :style='{ "padding": "2px 10px", "borderColor": "#eee", "borderStyle": "dashed", "borderWidth": "0 0 1px 0" }'
									class="like">
								...
								<div :style='{ "padding": "2px 10px", "borderColor": "#f0f0f0", "borderStyle": "dashed", "borderWidth": "0 0 1px 0" }'
									class="view">
			...
			<el-pagination background id="pagination" class="pagination" :pager-count="7" :page-size="pageSize"
				...
				@current-change="curChange" @size-change="sizeChange" @prev-click="prevClick"
...
export default {
	...
	created() {
		...
		this.$http.get('option/course_type/course_type').then(res => {
			if (res.data.code == 0) {
				this.courseTypeOptions = res.data.data;
				this.selectOptionsList.push({ name: '课程类型', list: this.courseTypeOptions, tableName: 'courseType', check: -1 })
			}
		});
		this.getFenlei();
		this.getList(1, '全部');
	...
	methods: {
		...
		getList(page, ref = '') {
			...
			let searchWhere = {};
			if (this.formSearch.kechengmingcheng != '')
				searchWhere.kechengmingcheng = '%' + this.formSearch.kechengmingcheng + '%';
			if (this.formSearch.courseType != '')
				searchWhere.kechengleixing = this.formSearch.courseType;
			if (this.formSearch.kechengkeshi != '')
				searchWhere.kechengkeshi = '%' + this.formSearch.kechengkeshi + '%';

			let user = JSON.parse(localStorage.getItem('sessionForm'))
			if (this.sortType)
				searchWhere.sort = this.sortType
			if (this.sortOrder)
				searchWhere.order = this.sortOrder
