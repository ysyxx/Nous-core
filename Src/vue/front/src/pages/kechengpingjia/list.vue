<template>
	<div class="course-evaluation-list">
		<!-- 面包屑导航区 -->
		...
		<!-- 返回按钮区（管理员可见） -->
		<div v-if="centerType" class="back-button-container">
			<el-button class="back-button" size="mini" @click="backClick" icon="el-icon-back" type="warning">返回</el-button>
		</div>
		...
		<div class="main-content">
			...
			<el-form :inline="true" :model="formSearch" class="search-form">
				...
				<el-button v-if="btnAuth('kechengpingjia', '新增')" class="add-btn" type="primary"
					@click="add('/index/kechengpingjiaAdd')">
			...
			<div class="filter-options">
				<div class="filter-group" v-for="(item, index) in selectOptionsList" :key="item">
					...
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)"
						...
						<div class="course-image">
							<img @click.stop="imgPreView(item.kechengtupian)"
								...
								:src="item.kechengtupian.split(',')[0]" class="image" />
						...
						<div class="course-info">
							<div class="basic-info">
								<div class="course-title">{{ item.kechengmingcheng }}</div>
								<div class="course-type">{{ item.kechengleixing }}</div>
								<div v-if="item.price" class="course-price">
							...
							<!-- 评价内容 -->
							<div class="evaluation-content" v-html="item.pingjiajianyi"></div>
						</div>
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		getList(page, fenlei, ref = '') {
			...
			this.$http.get(`courseAssessment/${this.centerType ? 'page' : 'list'}`, { params: Object.assign(params, searchWhere) }).then(res => {
				if (res.data.code == 0) {
...
@media (min-width: 768px) {
	.course-list .list-container .course-item {
		width: 48%;
	}
}
