<template>
	<div class="user-list-container">
		...
		<!-- 返回按钮 -->
		<div class="back-button-wrapper" v-if="centerType">
		...
		<div class="list-content-wrapper">
			...
			<el-form :inline="true" :model="formSearch" class="search-form">
				<el-form-item class="search-form-item">
					<div class="search-label">用户账号：</div>
					<el-input v-model="formSearch.yonghuzhanghao" placeholder="用户账号"
						@keydown.enter.native="getList(1, curFenlei)" clearable>
					</el-input>
				</el-form-item>
				<el-form-item class="search-form-item">
					<div class="search-label">用户姓名：</div>
					<el-input v-model="formSearch.yonghuxingming" placeholder="用户姓名"
						@keydown.enter.native="getList(1, curFenlei)" clearable>
					</el-input>
				</el-form-item>
				...
				<el-button v-if="btnAuth('yonghu', '新增')" class="add-button" type="primary"
					@click="add('/index/yonghuAdd')">
			...
			<div class="user-list-wrapper">
				<div class="user-list-content">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="user-item">
						<div class="user-avatar">
							<img @click.stop="imgPreView(item.touxiang)"
								v-if="item.touxiang && item.touxiang.substr(0, 4) == 'http'"
								:src="item.touxiang.split(',')[0]" class="avatar-image">
							<img @click.stop="imgPreView(baseUrl + (item.touxiang ? item.touxiang.split(',')[0] : ''))"
								v-else :src="baseUrl + (item.touxiang ? item.touxiang.split(',')[0] : '')"
								class="avatar-image">
						</div>
						<div class="user-info">
							<div class="info-main">
								<div class="user-name">{{ item.yonghuxingming }}</div>
								<div v-if="item.price" class="user-price"><span>￥</span>{{ item.price }}</div>
								<div class="user-time">
